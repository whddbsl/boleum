...
class NaverCallbackAPIView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            print(f'카카오 접근 토큰 : {access_token}')

            # User info get request (즉 로그인 한 사용자 정보 받아오기)
            ...
            # User 정보를 가지고 오는 요청에서 잘못이 없는 경우
            # user_info -> 카카오에서 가져온 유저 정보
            ...
            # User 의 email 을 받아오지 못한 경우 / 일치하지않은경우
            if email is None:
                return JsonResponse({
            ...
            try:
                ...
                return Response({'token' : accept_json['key'],
                                 'name' : name
            ...
            # 회원가입 처리를 해야 되는 유저
                ...
                accept_json = accept.json()
                print(f'이게 장고 내부에서 만들어주는 유저 토큰 : {accept_json}')
